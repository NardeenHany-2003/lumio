document.addEventListener('DOMContentLoaded', () => {
  ...
  if (saveAccountBtn) {
    saveAccountBtn.addEventListener('click', async () => {
      ...
      if (!name || !email)
        return showFormAlert('account-alert', 'error', 'Name and email are required.');
      ...
      try {
        const res  = await fetch('/api/v1/users/update-me', { method: 'PATCH', body: formData });
        ...
        if (updatedUser.photo) {
          ...
          document
            .querySelectorAll('.navbar__avatar, .dashboard__avatar, .profile-photo-preview')
            .forEach((img) => { img.src = newSrc; });
        }
        document
          .querySelectorAll('.navbar__user-name, .dashboard__profile h3')
          .forEach((el) => { el.textContent = updatedUser.name; });
      } catch (err) {
  ...
  if (savePasswordBtn) {
    savePasswordBtn.addEventListener('click', async () => {
      ...
      if (!passwordCurrent || !password || !passwordConfirm)
        return showFormAlert('password-alert', 'error', 'Please fill in all password fields.');
      if (password.length < 8)
        return showFormAlert('password-alert', 'error', 'New password must be at least 8 characters.');
      if (password !== passwordConfirm)
        return showFormAlert('password-alert', 'error', 'New passwords do not match.');
  ...
  // Deactivate Account
  // Must be inside DOMContentLoaded so setLoading is in scope
  const deleteAccountBtn = document.querySelector('#delete-account-btn');
  if (deleteAccountBtn) {
    deleteAccountBtn.addEventListener('click', async () => {
      const confirmed = await lumioConfirm({
        title:        'Deactivate your account?',
        message:      'Your account will be deactivated immediately. You can contact support to reactivate it.',
      ...
      setLoading(deleteAccountBtn, true, ' Deactivating…', 'bi bi-trash3');
      try {
        ...
        setLoading(deleteAccountBtn, false, ' Deactivate My Account', 'bi bi-trash3');
      }
    });
  }
